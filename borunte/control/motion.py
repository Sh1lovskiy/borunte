...
from .controller import action_single_cycle, modify_gspd, start_button
...
def write_and_run_point(client: RobotClient, pose: Sequence[float], max_retries: int = 2) -> bool:
    """
    ...
    while True:
        ...
        if time.time() - last >= 1.0:
            ...
            _log.tag(
                "MOVE",
                f"mode={mode} moving={moving} alarm={alarm} W=({wx}, {wy}, {wz}, {wu}, {wv}, {ww})",
            )
            last = time.time()
        if moving == 0:
            break
