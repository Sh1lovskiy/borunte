...
class RobotClient:
    ...
    def _recv_json(
        self, timeout: Optional[float] = None, allow_interrupt: bool = False
    ...
    def query(self, keys: List[str], timeout: Optional[float] = None) -> Optional[List[str]]:
    ...
    def _read_readDataList(self, start: int, count: int) -> Optional[List[int]]:
    ...
    def _read_query_addrs(self, start: int, count: int) -> Optional[List[int]]:
    ...
    def _read_addressing(self, start: int, count: int) -> Optional[List[int]]:
    ...
    def get_world_pose(self) -> Optional[Tuple[float, ...]]:
        vals = self.query(
