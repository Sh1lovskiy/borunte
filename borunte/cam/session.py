...
class CaptureSession:
    ...
    def __init__(
        ...
        base = Path(root_dir) if root_dir is not None else Path(config.paths.captures_root)
    ...
    def save_params_json(
        ...
        def intr_to_dict(intr) -> dict[str, float]:
            return {
                ...
                "coeffs": [float(c) for c in list(getattr(intr, "coeffs", [0, 0, 0, 0, 0]))[:5]],
            }

        def extr_to_dict(extr) -> dict[str, list[float]]:
            rot = [float(x) for x in getattr(extr, "rotation", [1, 0, 0, 0, 1, 0, 0, 0, 1])]
        ...
        try:
            depth_profile = profile.get_stream(2).as_video_stream_profile()  # rs.stream.depth
            color_profile = profile.get_stream(1).as_video_stream_profile()  # rs.stream.color
    ...
    def _save_png(self, path: Path, img: np.ndarray) -> None:
        ...
        try:
            ...
            if cv2 is not None and hasattr(cv2, "imwrite"):
                cv2.imwrite(str(path), img[:, :, ::-1] if img.ndim == 3 else img)  # BGR/RGB
            else:
                raise
