...
class PreviewStreamer:
    ...
    def __init__(
        ...
        disparity_shift: Optional[int] = None,
    ) -> None:
        self.config = config
        self.view = view or config.preview.view
        self.disparity_shift = (
            config.preview.disparity_shift
            if disparity_shift is None
    ...
    def _open(self) -> None:
        ...
        if want_depth and self.disparity_shift is not None:
            try:
                applied = _set_and_verify_disparity_shift(
                    device, int(self.disparity_shift)
                )
                _log.tag("RS2", f"preview disparityShift={applied}")
            except Exception as exc:
                ErrorTracker.report(exc)
                _log.tag(
                    "RS2", f"preview disparityShift not applied: {exc}", level="warning"
                )
        ...
        if self.profile and not self._logged_intrinsics:
            try:
                ...
                d_stream = self.profile.get_stream(
                    rs.stream.depth
                ).as_video_stream_profile()
                c_stream = self.profile.get_stream(
                    rs.stream.color
                ).as_video_stream_profile()
                _log.tag(
                    "RS2",
