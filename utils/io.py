...
def load_intrinsics_from_json(
    ...
    """
    Read intrinsics JSON (RealSense format) from capture root.
    Returns (w, h, fx, fy, cx, cy) for color stream.
    Falls back to provided defaults if file is missing or invalid.
    """
    ...
    try:
        ...
        color = data.get("intrinsics", {}).get("color", {})
        ...
        logger.info(
            f"[INTR JSON] {p.name}: "
            f"w={w} h={h} fx={fx:.3f} fy={fy:.3f} "
            f"cx={cx:.3f} cy={cy:.3f}"
        )
        return w, h, fx, fy, cx, cy

    except Exception as e:
        logger.warning(f"[INTR JSON] failed to parse {p.name}: {e}")
        return (
            fallback.width,
            fallback.height,
            fallback.fx,
