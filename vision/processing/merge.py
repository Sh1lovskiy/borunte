...
class CloudMerger:
    ...
    def _register(self, base: o3d.geometry.PointCloud, cloud: o3d.geometry.PointCloud) -> np.ndarray:
    ...
    def merge(self, clouds: Sequence[o3d.geometry.PointCloud]) -> MergeResult:
        ...
        for index, cloud in enumerate(progress_bar(range(len(clouds)), description="Merging frames")):
            ...
            if base_cloud is None:
                ...
                if self.visualize_per_frame and viewer is not None and index % self.visualize_every_k == 0:
        ...
        merged_points = np.concatenate(accumulated_points) if accumulated_points else np.empty((0, 3))
