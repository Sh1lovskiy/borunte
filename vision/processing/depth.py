...
def depth_to_xyz_with_mask(depth: np.ndarray, config: VisionConfig) -> Tuple[np.ndarray, np.ndarray]:
...
def depth_to_cloud(depth: np.ndarray, color: Optional[np.ndarray], config: VisionConfig) -> o3d.geometry.PointCloud:
...
def create_cloud(points: np.ndarray, colors: Optional[np.ndarray] = None) -> o3d.geometry.PointCloud:
